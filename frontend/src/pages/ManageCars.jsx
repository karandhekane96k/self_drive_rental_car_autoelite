...
export default function ManageCars() {
  ...
  const fetchCars = async () => {
    try {
      const response = await fetch('https://self-drive-rental-car-Nandi Cars.onrender.com/api/cars?admin=true');
      ...
      if (imageFile) {
        ...
        if (uploadRes.ok && returnedPath) {
          imageUrl = returnedPath.startsWith('http') 
            ? returnedPath 
            : `https://self-drive-rental-car-Nandi Cars.onrender.com${returnedPath}`;
        } else {
          throw new Error(uploadResult.message || 'Image upload failed');
        }
      }

      const response = await fetch(`https://self-drive-rental-car-Nandi Cars.onrender.com/api/cars/${editingCar._id}`, {
        method: 'PUT',
  ...
  const handleToggleVisibility = async (car) => {
    try {
      const response = await fetch(`https://self-drive-rental-car-Nandi Cars.onrender.com/api/cars/${car._id}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(`https://self-drive-rental-car-Nandi Cars.onrender.com/api/cars/${id}`, { method: 'DELETE' });
      if (response.ok) {
